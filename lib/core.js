...
module.exports = {
    'set': {
        ...
        process: function (data, global, state, variables, result, params, args, jsVariables) {
            if (params.name[0] !== '$') {
                console.log('error: variable name must start with $.');
                return data;
            }
            try {
                _.each(jsVariables, function (v, label) {
                    params.value = replaceAll(params.value, label, JSON.stringify(v));
                });
                var v = eval('[' + params.value + ']');
            } catch (e) {
                console.log('error.');
                return;
            }
    ...
    'get': {
        ...
        description: '変数の値を表示する。',
        process: function (data, global, state, variables, result, params, args, jsVariables, originalArgs) {
            console.log([originalArgs[0], params.name].join(': '));
            return data;
        }
    },
    'log': {
    ...
    'show_variables': {
        args: [],
        description: '変数の一覧を表示する。',
        process: function (data, global, state, variables, result, params, args, jsVariables) {
            _.each(jsVariables, function (v, k) {
                var s = v.toString();
